...
import { getAllAppointments, SOCKET_URL, adminGetClinics, adminCreateClinic, adminUpdateClinic, adminDeleteClinic, adminGetUsers, adminCreateUser, adminUpdateUser, adminDeleteUser, adminGetAuditLogs, adminApproveUser, adminGetSettings, adminUpdateSettings, adminGetDoctorStats } from '../services/api';
import { Calendar, Search, BarChart2, PieChart, Users, CheckSquare, XCircle, Download, Filter, FileText as FileIcon, Globe } from 'lucide-react';
...
export const AdminDashboard: React.FC = () => {
    ...
    const [userForm, setUserForm] = useState({ username: '', full_name: '', role: 'staff', password: '', confirm: '', clinic_id: '' });
    ...
    const [userErrors, setUserErrors] = useState<{ username?: string; full_name?: string; role?: string; password?: string; confirm?: string }>({});
    ...
    const byClinic = filtered.reduce((acc: any, curr) => {
        acc[curr.clinic_name] = (acc[curr.clinic_name] || 0) + 1;
        return acc;
    }, {});
    ...
    const statusData = [
        ...
        { name: 'Waiting/Booked', value: waiting, color: '#3B82F6' }, // blue-500
    ...
    const exportPDF = async () => {
        ...
        doc.text(`Completed: ${completed}`, 14, 66);
        doc.text(`Waiting: ${waiting}`, 14, 72);
        doc.text(`Cancelled/No-show: ${cancelled + noShow}`, 14, 78);
        doc.text(`Walk-ins: ${walkInCount}`, 80, 60);
        doc.text(`Scheduled: ${scheduledCount}`, 80, 66);
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                    ...
                    <div className="bg-white p-6 rounded-lg shadow-sm border-l-4 border-yellow-500">
                         <div className="flex items-center justify-between">
                            <div>
                                <p className="text-sm text-gray-500">Waiting/In-Progress</p>
                        ...
                        <table className="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                                {loading ? (
                                    ...
                                    <tr><td colSpan={6} className="px-6 py-8 text-center text-gray-500">No appointments found.</td></tr>
                                ) : (
                                    filtered.map((apt) => (
                                        <tr key={apt.id} className="hover:bg-gray-50">
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <div className="text-sm font-medium text-gray-900">{apt.patient?.full_name}</div>
                        ...
                        <div className="bg-white rounded-lg shadow-sm p-6">
                            ...
                            <div className="space-y-3">
                                ...
                                <button
                                  className="px-4 py-2 bg-green-600 text-white rounded disabled:opacity-50"
                                  onClick={async()=>{
                                    const errors: { username?: string; full_name?: string; role?: string; password?: string; confirm?: string } = {};
                                    ...
                                    if (Object.keys(errors).length > 0) return;
                                    const payload: any = { username: userForm.username.trim(), full_name: userForm.full_name.trim(), role: userForm.role as any, password: userForm.password };
                                    if (userForm.clinic_id) payload.clinic_id = userForm.clinic_id;
                                    await adminCreateUser(payload);
                                    setUserForm({ username: '', full_name: '', role: 'staff', password: '', confirm: '', clinic_id: '' });
                                    loadAdminData();
                                  }}
                                >Create</button>
                            </div>
                    ...
                    <div className="bg-white rounded-lg shadow-sm p-6">
                        ...
                        <div className="overflow-x-auto">
                            <table className="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody className="bg-white divide-y divide-gray-200">
                                    {logs.map((l:any)=>(
                                        <tr key={l.id}><td className="px-4 py-2 text-sm text-gray-500">{new Date(l.timestamp).toLocaleString()}</td><td className="px-4 py-2 text-sm text-gray-900">{l.actor || '-'}</td><td className="px-4 py-2 text-sm">{l.action}</td><td className="px-4 py-2 text-sm text-gray-500">{typeof l.details==='object'?JSON.stringify(l.details):String(l.details)}</td></tr>
                                    ))}
