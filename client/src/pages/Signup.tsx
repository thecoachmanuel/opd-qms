...
export const Signup: React.FC = () => {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (!validRoles.includes(form.role as any)) { setError('Role must be admin, staff, or doctor'); return; }
    if (!form.clinic_id.trim()) { setError('Please select a clinic'); return; }
    try {
      const resp = await authSignup({ username: form.username.trim(), full_name: form.full_name.trim(), role: form.role as any, clinic_id: form.clinic_id, password: form.password, email: form.email.trim() || undefined, phone: form.phone.trim() || undefined });
      if (resp.approved) {
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
      <div className="max-w-md w-full bg-white rounded-lg shadow-md p-8">
        ...
        <form onSubmit={submit} className="space-y-4">
          ...
          <p className="text-xs text-gray-500 mb-1">Select your designation: Staff, Doctor, or Admin.</p>
          ...
          <input className="w-full border rounded p-2" placeholder="Email (optional)" value={form.email} onChange={e=>setForm({...form, email:e.target.value})} />
