  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (resp.role === 'admin') {
        ...
        navigate('/staff');
      } else if (resp.role === 'doctor') {
        navigate('/doctor');
      } else {
        // Fallback for unknown roles
        console.warn('Unknown role logged in:', resp.role);
        navigate('/');
      }
    } catch (err: any) {
      ...
      } else if (err.message?.includes('JSON') || err.code === 'PGRST116') {
        // PGRST116 is "The result contains 0 rows" (Profile not found)
        setError('Account exists but profile data is missing. Please contact support.');
      } else if (err.message?.includes('Database error querying schema') || err.code === 'PGRST200' || err.code === 'unexpected_failure') {
        setError(`System error (Code: ${err.code}). Please DELETE this user from the Admin Dashboard and CREATE IT AGAIN.`);
